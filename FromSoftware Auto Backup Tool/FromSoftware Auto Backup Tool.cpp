...
// Release # 1.1
// Changelog: https://www.notion.so/FromSoftware-games-auto-backup-tool-fdb275a157c74e49bd57a752bf90a6a6
...
int main()
{
	...
	string DSSAVE = "DRAKS0005.sl2";                // Name of initital save if Dark Souls : Prepare To Die Edition
	string DSRSAVE = "DRAKS0005.sl2";               // Name of initital save if Dark Souls : Remastered
	string DS2SAVE = "DARKSII0000.sl2";             // Name of initital save if Dark Souls II
	string DS2SOFSSAVE = "DS2SOFS0000.sl2";         // Name of initital save if Dark Souls II: Scholar of the First Sin
	...
	string GAMESAVE;
	string BACKUP;
	string fsgame;					// Which game are you playing
	string fsgamename;				// Name of game which are you playing
	string dsVersion;
	...
	while (fsgame.empty()) {
		...
		cout << "FromSoftware Auto Backup Tool V1.1" << endl;
		cout << "-------------------------------------" << endl;
	...
	numberSaves = GetPrivateProfileIntA("FSABT", "max_saves", 5, ".\\settings.ini");	// Get the number of saves
	interval = GetPrivateProfileIntA("FSABT", "interval_minutes", 10, ".\\settings.ini");	// Get the frequency of saves																	

	if (fsgame == "1")
	{
		//Dark Souls
		GetPrivateProfileStringA("FSABT", "save_path_ds", "failed", pathbuffer, 100, ".\\settings.ini");       // Get the path of the save file
	...
	else if (fsgame == "1R")
	{
		...
		GetPrivateProfileStringA("FSABT", "save_path_dsr", "failed", pathbuffer, 100, ".\\settings.ini");       // Get the path of the save file
	...
	{
		//Dark Souls 2
		GetPrivateProfileStringA("FSABT", "save_path_ds2", "failed", pathbuffer, 100, ".\\settings.ini");       // Get the path of the save file
	...
	else if (fsgame == "2S")
	{
		...
		GetPrivateProfileStringA("FSABT", "save_path_ds2S", "failed", pathbuffer, 100, ".\\settings.ini");       // Get the path of the save file
	...
	{
		//Dark Souls 3
		GetPrivateProfileStringA("FSABT", "save_path_ds3", "failed", pathbuffer, 100, ".\\settings.ini");       // Get the path of the save file
	...
	{
		//Elden Ring"
		GetPrivateProfileStringA("FSABT", "save_path_ER", "failed", pathbuffer, 100, ".\\settings.ini");       // Get the path of the save file
	...
	else if (fsgame == "S")
	{
		...
		GetPrivateProfileStringA("FSABT", "save_path_S", "failed", pathbuffer, 100, ".\\settings.ini");       // Get the path of the save file
	...
	else if (fsgame == "A")
	{
		...
		GetPrivateProfileStringA("FSABT", "save_path_AC6", "failed", pathbuffer, 100, ".\\settings.ini");       // Get the path of the save file
